...
r = requests.get('http://tbgforums.com/forums/viewtopic.php?id=170') # get website info
...
pages = int(tbgSoup.select('.pagelink')[0].text.split(' ')[-2]) # number of pages

# Go through each page and get all the users that posted on that page.
for i in range(pages):
    r = requests.get('http://tbgforums.com/forums/viewtopic.php?id=170&p=%s' % str(i+1)) # get the specific page
...
data = dict(Counter(posters)) # Make a counter for every occurrence of every person
print()
post = []
...
# Add gold, silver, and bronze colors for first, second, and third place, respectively.
